...
function suctionCheck(inp, vSuction, lossesSuction) {
  ...
  // Допустимая высота всасывания с запасом 1.5 м и потерями
  const requiredLift = inp.depth + lossesSuction + vSuction*vSuction/(2*G) + 1.5;
  return { liftMax, requiredLift, ok: requiredLift <= liftMax };
}
...
const FORMULAS = {
  ...
  H: {
    ...
    note:
      `Значение по умолчанию <b>55 м вод. ст.</b> взято с паспортной H–Q кривой ` +
      `HRMD-3B 81-17-40 (Royal IHC) для рабочей точки Q = 2700 м³/ч ` +
      `(0.75 м³/с) при 792 об/мин на чистой воде.<br>` +
      `По графику: p<sub>man</sub> ≈ 535 кПа → <b>54.6 м вод. ст.</b><br>` +
      `Ключевые точки кривой:<br>` +
      `• Q = 0 (шут-офф): <b>~70 м</b> вод. ст.<br>` +
      `• Q = 2400 м³/ч: <b>~58 м</b><br>` +
      `• Q = 2700 м³/ч: <b>~55 м</b><br>` +
      `• Q = 3240 м³/ч (макс.): <b>~47 м</b><br>` +
      `На пульпе фактический развиваемый напор снижается множителем HR (см. отдельный tooltip).`,
    src: "ATTACHMENT 1 — Pump graph, HRMD-3B 81-17-40, Royal IHC (ref. 93.159, GO 6154, 01-Nov-2019).",
  },
  N: {
    title: "Мощность на валу насоса",
    note:
      `Значение по умолчанию <b>505 кВт</b> — максимальная мощность на валу насоса ` +
      `при 792 об/мин (CT), указанная производителем на паспортной кривой HRMD-3B 81-17-40.<br>` +
      `Это лимит, который пульпа не должна превышать. Сравнивается с N<sub>тр</sub>.<br>` +
      `Если используется редуктор/ременная передача — мощность от двигателя выше на величину ` +
      `1/η<sub>ред</sub> (см. поле «КПД редуктора» и результирующую ячейку «Мощность от двигателя»).`,
    src: "ATTACHMENT 1 — Pump graph, HRMD-3B 81-17-40 (Royal IHC).",
  },
  etaGear: {
...
function onAnyChange() {
  ...
  // показать поле "ось вращения" только для погружного
  document.getElementById("rowAxisDepth").hidden = (inp.pumpType !== "submerged");
